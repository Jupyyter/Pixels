...
void RigidBodySystem::renderToParticleWorld(ParticleWorld* particleWorld)
{
    ...
    {
        if (!rbData->isActive || B2_IS_NULL(rbData->bodyId)) continue;
        ...
        for (const auto& pixel : rbData->previousPixels)
        {
            ...
            {
                const auto& currentParticle = particleWorld->getParticleAt(pixel.x, pixel.y);
                ...
                {
                    particleWorld->setParticleAt(pixel.x, pixel.y, Particle::createEmpty());
                }
            }
        }
        ...
        switch (rbData->shape)
        {
                ...
                for (int dy = -static_cast<int>(rbData->radius); dy <= static_cast<int>(rbData->radius); ++dy)
                {
                    for (int dx = -static_cast<int>(rbData->radius); dx <= static_cast<int>(rbData->radius); ++dx)
                    {
                        ...
                        if (distance <= rbData->radius && particleWorld->inBounds(x, y))
                        {
                            ...
                            {
                                Particle p = particleWorld->createParticleByType(rbData->materialType);
                                p.color = rbData->color;
                                p.lifeTime = -1.0f; // Mark as rigid body particle
                                particleWorld->setParticleAt(x, y, p);
            ...
            case RigidBodyShape::Triangle:
            {
                ...
                for (int y = static_cast<int>(minY); y <= static_cast<int>(maxY); ++y)
                {
                    ...
                    {
                        if (!particleWorld->inBounds(x, y)) continue;
...
float RigidBodySystem::getMaterialDensity(MaterialID material) const
{
    switch (material)
    {
        ...
        case MaterialID::Sand:  return 1.5f;
        default:               return 1.0f;
    }
}
...
float RigidBodySystem::getMaterialFriction(MaterialID material) const
{
    switch (material)
    {
        case MaterialID::Stone: return 0.7f;
        case MaterialID::Wood:  return 0.5f;
        case MaterialID::Sand:  return 0.6f;
        default:               return 0.3f;
    }
}
...
float RigidBodySystem::getMaterialRestitution(MaterialID material) const
{
    switch (material)
    {
        case MaterialID::Stone: return 0.3f;
        case MaterialID::Wood:  return 0.1f;
        case MaterialID::Sand:  return 0.05f;
        default:               return 0.2f;
    }
}
